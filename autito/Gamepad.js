...
class Gamepad {
  ...
  _buttonsHaveBeenUpdated(newButtonsStatus) {
    return (
      JSON.stringify(this._status.buttons) !== JSON.stringify(newButtonsStatus)
    )
  }
  ...
  _thumbstickHaveBeenUpdated(thumbstick, newThumbstickStatus) {
    return (
      JSON.stringify(this._status.thumbsticks[thumbstick]) !==
        JSON.stringify(newThumbstickStatus[thumbstick]) &&
      this._isThumbstickOverNoiseThreshold(newThumbstickStatus)
    )
  }

  _getButtonsDifference(newButtons) {
    const difference = {}
    for (const key in this._status.buttons) {
      if (this._status.buttons[key] !== newButtons[key]) {
        difference[key] = newButtons[key]
      }
    }
    return difference
  }
  ...
  async start() {
    ...
    await page.exposeFunction('updateControllerStatus', (newStatus) => {
      if (
        ...
        const buttonsDifference = this._getButtonsDifference(newStatus.buttons)
        this._status = newStatus

        this._controllerEvent.emit('gamepadupdated', {
          newStatus,
          buttonsDifference,
        })
      }
